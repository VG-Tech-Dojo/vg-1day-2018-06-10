...
func MessagesAll(db *sql.DB) ([]*Message, error) {
	...
	// Tutorial 1-1. ユーザー名を表示しよう
	rows, err := db.Query(`select id, body, username from message`) // データベースからユーザー名をとる
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		m := &Message{}
		// Tutorial 1-1. ユーザー名を表示しよう
		if err := rows.Scan(&m.ID, &m.Body, &m.Name); err != nil {
			return nil, err
		}
		ms = append(ms, m)
	}
...
func MessageByID(db *sql.DB, id string) (*Message, error) {
	...
	// Tutorial 1-1. ユーザー名を表示しよう
	if err := db.QueryRow(`select id, body, username from message where id = ?`, id).Scan(&m.ID, &m.Body, &m.Name); err != nil {
		return nil, err
	}
...
func (m *Message) Insert(db *sql.DB) (*Message, error) {
	...
	res, err := db.Exec(`insert into message (body, username) values (?,?)`, m.Body, m.Name)
	if err != nil {
		return nil, err
	}
